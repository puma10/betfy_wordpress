...
$results = DB::query("SELECT rank,comments.*,top_listing.name as topname FROM `comments`,top_listing WHERE id_post=top_listing.id ORDER BY date DESC");
?>
        <!-- Page Content -->
        <div id="page-wrapper">
            <div class="row">
		...
		<div class="span5">
			...
			<?php 	foreach ($results as $row) { ?>		
                <tr>
                    <td><b><?php echo '(#'.$row['rank'].') '.$row['topname']; ?></b></td>
					<td><?php echo $row['name']; ?></td>
					<td><?php echo $row['email']; ?></td>
					<td><?php echo substr($row['comment'],0,100).'...'; ?></td>
					<td><span class="label label-success"><?php echo $row['rating'].'/5'; ?></span></td>
